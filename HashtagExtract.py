...
def Make_Panda_DataFrame_For_Each_User (df):
    ...
    return grouper
#----------------------
def hashtag_finder (df_pre, df_within, df_post):
    
    hashtag_list_pre = []
    hashtag_list_within = []
    hashtag_list_post = []
    i = 0
    while i < df_pre.shape[0]:
        if df_pre['hashtags'].iloc[i]:
            hashtags = df_pre['hashtags'].iloc[i].split("\t") #because tweets with more than one hashtags have their hashtags seperated with a tab which is marked with a '\t' : for a example a tweet with three hashtags #Amir and #Roya and #Reza has it as 'Amir\tRoya\tReza'
            for hashtag in hashtags:
                hashtag_list_pre.append (hashtag)
        i+=1

    i = 0
    while i < df_within.shape[0]:
        if df_within['hashtags'].iloc[i]:
            hashtags = df_within['hashtags'].iloc[i].split("\t") #because tweets with more than one hashtags have their hashtags seperated with a tab which is marked with a '\t' : for a example a tweet with three hashtags #Amir and #Roya and #Reza has it as 'Amir\tRoya\tReza'
            for hashtag in hashtags:
                hashtag_list_within.append (hashtag)
        i+=1

    i = 0
    while i < df_post.shape[0]:
        if df_post['hashtags'].iloc[i]:
            hashtags = df_post['hashtags'].iloc[i].split("\t") #because tweets with more than one hashtags have their hashtags seperated with a tab which is marked with a '\t' : for a example a tweet with three hashtags #Amir and #Roya and #Reza has it as 'Amir\tRoya\tReza'
            for hashtag in hashtags:
                hashtag_list_post.append (hashtag)
        i+=1
    
    return hashtag_list_pre, hashtag_list_within, hashtag_list_post
#----------------------
...
def Chunk_Chunk_Analysis (grouper, i):
    j = 5000*(i)
    hashtag_AllUsers_pre = []
    hashtag_AllUsers_within = []
    hashtag_AllUsers_post = []
    if j != 80000: # Early chunks contains 5k users each
        while j < 5000*(i+1):
            ...
            hashtag_list_pre, hashtag_list_within, hashtag_list_post = hashtag_finder (df_pre, df_within, df_post)
            hashtag_AllUsers_pre += hashtag_list_pre 
            hashtag_AllUsers_within += hashtag_list_within
            hashtag_AllUsers_post += hashtag_list_post
    ...
    hashtag_user_all = [hashtag_AllUsers_pre, hashtag_AllUsers_within, hashtag_AllUsers_post]
    print ("This chunk is finished : " + str(i))

    file_name = "/home/abodaghi/Twitter_Project/Data_Processing/Results/HashtagExtract/" + str(i) + ".pkl"
